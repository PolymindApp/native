...
export default class DataSettingsScreen extends React.Component {
	...
	handleAddColumn() {
		const dataset = this.props.route.params.datasetContext.state.dataset;
		const column = new DatasetColumn({ name: '', dataset: dataset.id });
				...
				>
					<View style={{flex: 1}}>
						...
						<ScrollView style={{flex: 1}} keyboardShouldPersistTaps={'handled'}>
							...
							<List.Subheader style={{marginTop: 15}}>{I18n.t('dataset.settings.general')}</List.Subheader>
							<View style={{marginHorizontal: 10, borderRadius: 10, padding: 5, paddingTop: 15, backgroundColor: 'white'}}>
								...
								/>
								<Text style={styles.desc}>{I18n.t('dataSettings.titleDesc')}</Text>
							</View>
							...
							<View style={{marginVertical: 10, marginTop: 15}}>
								...
								<View style={{margin: 10, padding: 10, backgroundColor: 'white', borderRadius: 10}}>
									...
									/>
									<Text style={styles.desc}>{I18n.t('dataSettings.imageDesc')}</Text>
										...
										leftElement={() => (
											<View>
												...
												<Text style={{opacity: 0.3}}>
													{dataset.icon.substring(4).toUpperCase().substring(0, 15) || I18n.t('iconSelector.none')}
												</Text>
											</View>
										)}
										onChange={value => {dataset.icon = value; this.setState({dataset});}}
									/>
									<Text style={styles.desc}>{I18n.t('dataSettings.iconDesc')}</Text>
								</View>
							</View>
						</ScrollView>
